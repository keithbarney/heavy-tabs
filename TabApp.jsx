...
// Initial section creator
const createSection = (name = 'New Section') => ({
  id: generateId(),
  name,
  measures: 1,
  repeat: 1,
...
const createEmptyMeasure = (instrument, stringCount) => {
  if (instrument === 'drums') {
    const lines = instrumentConfigs.drums.lines.length;
    return Array(lines).fill(null).map(() => Array(CELLS_PER_MEASURE).fill('-'));
  }
  return Array(stringCount).fill(null).map(() => Array(CELLS_PER_MEASURE).fill('-'));
};
...
export default function TabApp() {
  ...
  const getTabData = useCallback((sectionId, instrument) => {
    ...
    if (!tabData[key]) {
      ...
      const measures = Array(section.measures).fill(null).map(() => createEmptyMeasure(instrument, stringCount));
      return measures;
    }
    return tabData[key];
  ...
  useEffect(() => {
    ...
    sections.forEach(section => {
      ['guitar', 'bass', 'drums'].forEach(instrument => {
        ...
        if (!newTabData[key]) {
          const stringCount = instrument === 'drums' ? instrumentConfigs.drums.lines.length : stringCounts[instrument];
          newTabData[key] = Array(section.measures).fill(null).map(() => createEmptyMeasure(instrument, stringCount));
        }
      });
    });
  ...
  const getCellsBetween = (start, end) => {
    ...
    for (let m = minMeasure; m <= maxMeasure; m++) {
      for (let s = minString; s <= maxString; s++) {
        ...
        const endCell = m === maxMeasure ? maxCell : CELLS_PER_MEASURE - 1;
  ...
  const inputValue = (value) => {
    ...
    if (!data[parseInt(measureIdx)]) return;
    
    const newData = data.map((measure, mIdx) => 
      measure.map((string, sIdx) => 
        string.map((cell, cIdx) => {
          if (mIdx === parseInt(measureIdx) && sIdx === parseInt(stringIdx) && cIdx === parseInt(cellIdx)) {
            return value;
          }
          return cell;
        })
    ...
    // Move to next cell
    const nextCellIdx = parseInt(cellIdx) + 1;
    if (nextCellIdx < CELLS_PER_MEASURE) {
      const nextKey = `${sectionId}-${measureIdx}-${stringIdx}-${nextCellIdx}`;
      setSelectedCells([nextKey]);
      setSelectionStart({ sectionId, measureIdx: parseInt(measureIdx), stringIdx: parseInt(stringIdx), cellIdx: nextCellIdx });
    }
  };
    ...
    switch (direction) {
      ...
      case 'ArrowLeft':
        if (cellIdx > 0) {
          ...
          newCell = CELLS_PER_MEASURE - 1;
        }
        break;
      case 'ArrowRight':
        if (cellIdx < CELLS_PER_MEASURE - 1) {
          newCell = cellIdx + 1;
  ...
  // Add section
  const addSection = () => {
    const newSection = createSection(`Section ${sections.length + 1}`);
    ...
    if (updates.measures !== undefined) {
      ...
      ['guitar', 'bass', 'drums'].forEach(instrument => {
        ...
        if (updates.measures > currentData.length) {
          const newMeasures = Array(updates.measures - currentData.length).fill(null).map(() => createEmptyMeasure(instrument, stringCount));
  ...
  const startPlayback = () => {
    ...
    const playStep = () => {
      ...
      // Advance position
      currentCell++;
      if (currentCell >= CELLS_PER_MEASURE) {
        currentCell = 0;
        currentMeasure++;
